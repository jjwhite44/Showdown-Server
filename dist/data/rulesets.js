...
const Rulesets = {
  ...
  testcustompokedex: {
    effectType: "ValidatorRule",
    name: "Test Custom Pokedex",
    desc: "Only allows Pok&eacute;mon native to gens 1-4",
    onValidateSet(set, format) {
      const customdex = [
        "Rillaboom",
        "Quilladin",
      ...
      const species = this.dex.species.get(set.species || set.name);
      if (!customdex.includes(species.baseSpecies) && !this.ruleTable.has("+" + species.id)) {
        return [`${species.name} is not in the custom Pok\xE9dex.`];
      }
    }
  },
  oldalolapokedex: {
